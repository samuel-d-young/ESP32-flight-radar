/*
  ...
  ESP8266 / ESP32 / ESP32-S3  — v1.4.0  Improv WiFi
*/
#include <Arduino.h>
...
void showMessage(const char* l1, const char* l2="", const char* l3=""){
  ...
  if(strlen(l1)) tft.drawString(l1,SCREEN_W/2,SCREEN_H/2-14);
  tft.setTextColor(TFT_GREEN,TFT_BLACK);
  if(strlen(l2)) tft.drawString(l2,SCREEN_W/2,SCREEN_H/2+2);
  tft.setTextColor(TFT_DARKGREY,TFT_BLACK);
  if(strlen(l3)) tft.drawString(l3,SCREEN_W/2,SCREEN_H/2+18);
}
...
bool connectSaved(){
  ...
  WiFi.begin((char*)savedSSID,(char*)savedPass);
  unsigned long t=millis();
